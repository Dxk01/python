...
class SequencePattern(object):
	...
	def __init__(self, name=None, in_seq_len=None, out_seq_len=None, embding_size=None):
		if name is not None:
		...
		if embding_size:
			self.OUTPUT_MAX_INT = embding_size
			self.INPUT_MAX_INT = embding_size
...
class TFLearnSeq2Seq(object):
	...
	def generate_test_date(self, num_points):
		'''
				Generate test dataset.  Produce random (integer) sequences X, and corresponding
				expected output sequences Y = generate_output_sequence(X).

				Return x_data, y_data (both of type uint32)

				x_data = numpy array of shape [num_points, in_seq_len]
				y_data  = numpy array of shape [num_points, out_seq_len]
		'''
		x_data = np.random.randint(0, self.in_max_int,
		                           size=(num_points, self.in_seq_len))  # shape [num_points, in_seq_len]
	...
	def train(self, X=None, Y=None, num_epochs=20, num_points=100000, model=None, model_params=None,
	          weights_input_fn=None,
...
def loadData():
	file = "/Users/orion/PycharmProjects/Test2/MyCode/Algorithm/Seq2SeqModel/data.txt"
	data = []
	# laod data from file
	with open(file, "r") as fp:
		for line in fp:
			iter = line.strip().split(" ")
			# print (iter[2:9])
			line_num = []
			for t in iter[2:9]:
				line_num.append(int(t))
			data.append(line_num)

	return data


def get_train_X_Y(data):
	train_X = data[:-1]
	train_Y = data[1:]
...
# # embedding_rnn embedding_attention
tf_seq = TFLearnSeq2Seq(sp, seq2seq_model="embedding_rnn")
# tf_seq.train(X=train_query_response[:100], Y=train_response[:100], num_epochs=5, batch_size=128)

tf_seq.train(X=train_query_response, Y=train_response, num_epochs=20, batch_size=512)
print("Start ... ...")

# id_word = None
# with open("id_word.pkl", "r") as fp:
# 	id_word = pkl.load(fp)


def getText(id_word, query_id, index):
	text = ""
	for w in x:
		if w == index:
			break
...
for x, y in zip(train_query[:1000], train_response[:1000]):
	# print(x)
	text = getText(id_word, x, index)
	print(u"ask:{}".format(text))
	prediction, y_pre = tf_seq.predict(Xin=x)
	print(u"real answer:{}".format(getText(id_word, y, index)))
	print(u"predict answer:{}".format(getText(id_word, prediction, index)))
#
# prediction, _ = tf_seq.predict(Xin=predict_X)
# print("cur {} next predict:{}".format(predict_X, prediction))
