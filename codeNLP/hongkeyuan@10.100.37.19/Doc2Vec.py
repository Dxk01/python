...
class  Doc2VecObj(object):
	...
	def buildModel(self,docs,type_t = 1):
		if type_t == 1:
			...
			self.model = Doc2Vec(docs,dm=1, dm_concat=1, size=100, window=2, hs=0, min_count=2,workers=2)
		elif type_t == 2:
			# PV-DBOW
			Doc2Vec(docs,dm=0, size=100, hs=0, min_count=2,workers=2)
		elif type_t == 3:
			# PV-DM w/average
			Doc2Vec(docs,dm=1, dm_mean=1, size=100, window=2, hs=0, min_count=2)
	...
	def calSimilarly(self,sentence):
		...
		sims = self.model.docvecs.most_similar([inferred_vector], topn=15)
		return sims
