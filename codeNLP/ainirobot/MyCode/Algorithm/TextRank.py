...
def handel_weibo_data():
    ...
    fp = open(MyCode.config.CorpusFilePath + 'corpus.txt', 'r')
...
def build_matrix():
    ...
    ########第四步，将排名前100的关键词写入文件
    fp_textrank_result = open(MyCode.config.ResultFilePath + 'result_textrank.txt', 'w+')
    for i in range(100):
...
def post_handel_textrank():
    # 读取排名前100的词和权值
    fp_result = open(MyCode.config.ResultFilePath + 'result_textrank.txt', 'r')
    ...
    fp_result.close()
    #读取原始微博连成字符串
    fp_weibo = open(MyCode.config.ResultFilePath + 'corpus.txt', 'r')
    weibo_list = []
