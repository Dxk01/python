...
def transformResult(pre_result=None,loc_dic=None):
    ...
    for result in pre_result:
        ...
        for i in xrange(3):
            ...
            topThree.append(min_index)
        line_result.append(topThree)

    for topThree in line_result:
        line_answer = []
        for cur_index in topThree:
            # if cur_index

            line_answer.append(place_list[cur_index][0])
        final_result.append(line_answer)
    return final_result
...
def main():
    ...
    # print "test data len:",length
    # print "predict result ... ... "
    # for i in xrange(200,length):
    #     pre_result = None
    #     start_index = i * 10000
    #     end_index = (i+1) * 10000
    #     if end_index > length:
    #         end_index = length
    #     datass = test_Xc[start_index:end_index]
        # for id in xrange(start_index,end_index):
        #     print test_Xc[id]
        ...
        # wr = WriteResult()
        # wr.WriteResultAnswer(test_data[start_index:end_index], result,Config.ResultDataPath+"result_2.csv")
        # end_time = time.time()
